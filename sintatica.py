...
class AnaliseSintatica:
    ...
    def p_expressao_escreva_1(self, p):
        'expressao_escreva : ESCREVE ABREPARENTESES expressao FECHAPARENTESES'
        p[0] = Arvore('declaracao_expressao_escreva', [p[3]])

    def p_sequencia_de_declaracao_1(self, p):
        'sequencia_de_declaracao : declaracao_codigo'
        p[0] = Arvore('sequencia_declaracao_declaracao', [p[1]])

    def p_sequencia_de_declaracao_2(self, p):
        'sequencia_de_declaracao : sequencia_de_declaracao declaracao_codigo'
        p[0] = Arvore('sequencia_de_declaracao_loop', [p[1], p[2]])

    def p_declara_var_1(self, p):
        'declara_var : tipo DOISPONTOS ID VIRGULA declara_var_loop'
...
def print_arvore(self, node, level="->"):
    if node != None:
...
if __name__ == '__main__':
    ...
    f = open(argv[1])
    sintatica = AnaliseSintatica(f.read())
    print_arvore(sintatica.ast)